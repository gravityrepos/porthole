...
**1418 tests, measured on ubuntu-latest CI** (a total holds on every leg; a
...
passed, 0 failed, 8 skipped), 729 in the MCP server (`cd mcp && npm test` —
726 passed, 0 failed, 3 skipped), and 245 in the timeline UI (`cd mcp && npm
...
XML, and when 1418 disagrees with the sum of the three suites' totals stated
...
set) and the same 729 server tests with 0 skipped, because it has the
cached `trace_processor` capture the ubuntu leg lacks; a worktree checkout
sees 729/727/2, missing only that capture. The timeline UI is the one suite
