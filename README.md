...
`release` refuses on a dirty tree, on any branch but `main`, on a version that
does not match `X.Y.Z` or `X.Y.Z-suffix`, and on an empty `## [Unreleased]`
section in [`CHANGELOG.md`](CHANGELOG.md) — each with a one-line reason.
Given a version, it:
